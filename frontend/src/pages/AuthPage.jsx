...
export default function AuthPage({ initialRegister = false }) {
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className="w-full min-h-screen text-slate-100 flex flex-col items-center justify-center relative font-sans overflow-x-hidden p-4 sm:p-6 lg:p-12"
      style={{
    ...
    >
      ...
      {/* Symmetric Centered Container */}
      <div className="w-full max-w-6xl mx-auto flex flex-col lg:flex-row items-start justify-center gap-8 lg:gap-12 z-10">
        
        {/* Left Column Brand Banner (desktop only, top-aligned) */}
        <div className="hidden lg:flex lg:w-1/2 flex-col justify-between p-8 lg:p-10 bg-[#16161d]/40 border border-white/[0.06] rounded-3xl min-h-[580px] select-none">
          ...
          <div className="mt-8 mb-auto max-w-lg">
            ...
            <h2 className="text-4xl lg:text-5xl font-bold tracking-tight leading-[1.1] mb-5">
              Your thoughts, <br />
            ...
            <div className="flex flex-col gap-3 mb-8">
              <div className="flex items-center gap-3 text-sm text-slate-300">
                <div className="w-5 h-5 rounded-full bg-indigo-500/10 border border-indigo-500/20 flex items-center justify-center text-indigo-400 text-xs">
                  ✦
                </div>
            ...
            <div className="p-4 bg-white/[0.02] border border-white/[0.04] rounded-2xl">
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div className="flex flex-col">
                  <span className="text-xs font-semibold text-slate-200">Jordan D.</span>
                  <span className="text-[10px] text-slate-400 font-medium">Early User & Daily Journaler</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="text-xs text-slate-500 font-medium pt-4">
            © {new Date().getFullYear()} MindVault. A Calm Space for Your Thoughts.
          </div>
        </div>
        ...
        <div className="w-full lg:w-1/2 flex items-start justify-center">
          ...
          >
            ...
            {/* Custom Tabs */}
            <div className="flex p-1 bg-black/20 border border-white/[0.04] rounded-2xl relative min-h-[44px]" role="tablist" aria-label="Authentication Options">
              <button
                role="tab"
                aria-selected={!isRegister}
                onClick={() => { setIsRegister(false); setError(''); }}
                className={`flex-1 py-3 text-sm font-semibold rounded-xl transition-all duration-300 z-10 cursor-pointer min-h-[44px] flex items-center justify-center ${
            ...
            {error && (
              <div className="flex items-center gap-2 p-3 text-xs text-rose-400 bg-rose-500/10 border border-rose-500/20 rounded-2xl" role="alert">
            ...
            <form onSubmit={handleAuth} className="flex flex-col gap-4">
              <div className="flex flex-col gap-1.5">
                <label htmlFor="username" className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Username</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="input input-icon-left text-slate-100"
                  />
                </div>
              </div>
              ...
              {isRegister && (
                <div className="flex flex-col gap-1.5 animate-fadeIn">
                  <label htmlFor="email" className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Email Address</label>
                  <div className="relative">
                    <Mail size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
                    <input
                      id="email"
                      name="email"
                      type="email"
              ...
              <div className="flex flex-col gap-1.5">
                <label htmlFor="password" className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Password</label>
                <div className="relative">
                  <Lock size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
                  <input
                    id="password"
                    name="password"
                    ...
                    className="input input-icon-left input-icon-right text-slate-100"
                  />
                  <button
                    type="button"
                    aria-label={showPassword ? 'Hide password' : 'Show password'}
                    onClick={() => setShowPassword(!showPassword)}
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-300 hover:text-white min-w-[36px] min-h-[36px] flex items-center justify-center cursor-pointer z-10"
                  >
              ...
              {isRegister && (
                <div className="flex flex-col gap-1.5 animate-fadeIn">
                  <label htmlFor="confirmPassword" className="text-[10px] font-bold text-indigo-300 uppercase tracking-wider">Confirm Password</label>
                  <div className="relative">
                    <Lock size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 z-10" />
                    <input
                      id="confirmPassword"
              ...
              {isRegister && (
                <label htmlFor="sentimentAnalysis" className="flex items-center gap-2.5 p-3 rounded-2xl bg-white/[0.02] border border-white/[0.04] cursor-pointer group mt-1 select-none min-h-[44px]">
                  <input
                    id="sentimentAnalysis"
                    type="checkbox"
                    checked={sentimentAnalysis}
                    onChange={(e) => setSentimentAnalysis(e.target.checked)}
                    className="rounded border-white/10 text-[#6366f1] focus:ring-[#6366f1]/20 cursor-pointer w-4 h-4"
                  />
              ...
              <button
                type="submit"
                disabled={loading}
                className="btn-primary w-full mt-2 min-h-[44px] flex items-center justify-center gap-2 font-bold cursor-pointer"
              >
            ...
            <button
              ...
              className="w-full min-h-[44px] flex items-center justify-center gap-2.5 px-4 py-2.5 rounded-xl bg-white/[0.03] hover:bg-white/[0.06] border border-white/10 text-slate-300 hover:text-white text-sm font-medium transition-all duration-200 cursor-pointer"
            >
